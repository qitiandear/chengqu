...
function delImg(id){
	if(confirm("是否删除该记录")){
		window.location = '/admin.php/Chengqu/delete/id/'+id;
	}
}
</script>
<script>
    ...
    console.log(vals);
      ...
      if(confirm("是否删除")){
        // document.getElementById("taken_").innerHTML = "<span style='color:green;'>已还书</span>";
         window.location = '/admin.php/Chengqu/deleted/id/'+vals;
        }
...
<h3 align="center">管理单位</h3><br>
	<table width="90%" border="1" align="center" style="text-align: center">
							...
							<?php if(is_array($arr)): foreach($arr as $k=>$v): ?><tr style="background-color:<?php echo $k%2==NULL?'#FFFFFF':'#F1F5FE';?>">
								<td><?php echo ($v["fname"]); ?></td>
								<td>
								<a href="/admin.php/Chengqu/update/id/<?php echo ($v["id"]); ?>">修改</a>
								<a href="javascript:void(0)" onclick="delImg(<?php echo ($v["id"]); ?>)">删除</a></td>
