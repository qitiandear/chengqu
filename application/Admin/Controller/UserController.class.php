<?php
	...
	class UserController extends BaseController{
		...
		public function upade()
		{
			...
			$typeId = $_GET['typeId'];
			$tel = $_GET['tel'];
			$result = M()->execute("update admin set username='{$username}',password='{$password}',cid='{$typeId}',tel='{$tel}' where id='{$id}'");
			if($result>0){
		...
		public function add(){
			//查找文章类型
			$types = M("types")->where("typeId<26 or typeId>34")->select();
		...
		public function adur(){
			if($_POST){
				...
				$result = M()->execute("insert into admin(username,password,cid,tel,qxId) value ('{$username}','{$password}','{$typeId}','{$tel}','{$qxId}')");
				if($result > 0){
		...
		public function lister(){
			...
			$arr = M()->query("select a.id,a.username,a.password,a.tel,c.fname from admin as a join chengqu as c on c.id=a.cid  where a.state<9 limit $page->firstRow,$page->listRows");
			$this->assign("arr",$arr);
