...
public function delete(){
	$id = $_GET['id'];
		...
		public function oper(){
			$info = M("tanchu")->where("state<9")->select();
			$this->assign("info",$info);
			$this->display();
		}
		public function adder(){
			$newsType = M("tanchu")->where("state<9")->select();
...
public function addnew()
		{
		    ...
		    $Path = $upload->uploadOne($myFile);
			if ($Path) {
				$result = M()->execute("insert into bless(content,title,edit,date,tcId,imagepath)	value  ('{$content}','{$title}','{$edit}','{$date}','{$tcId}','newspicture/{$Path['savename']}')");
		
			}else if ($imagepath==NULL)
			{
				$result = M()->execute("insert into bless(content,title,edit,date,tcId)	value  ('{$content}','{$title}','{$edit}','{$date}','{$tcId}')");
			}
